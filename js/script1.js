...
var coord = ''; 
var mType = '';
var mapWidth = 100 + '%'; var mapHeight = 400;
var mapZoom = 14;

function redrawMap() { var iframeUrl = generateMapUrl(); 
    var finalCode = getFinalWidgetCode(); 
    var finc = '<div style="width: 100%">' + finalCode + '</div>'; 
    return finc;
}

function getFinalWidgetCode() { var $iframe = getIFrame(); $iframe.append(getLinkBaitScript()); var finalWidgetCode = $iframe.wrap('<div />').parent().html(); return finalWidgetCode;

}

function getIFrame() { var $iframe = $('<iframe></iframe>'); $iframe.attr("width", mapWidth); $iframe.attr("height", mapHeight); $iframe.attr("frameborder", "0"); $iframe.attr("scrolling", "no"); $iframe.attr("marginheight", 0); $iframe.attr("marginwidth", 0); $iframe.attr("src", generateMapUrl()); 
return $iframe;

}

function getLinkBaitScript() { var z = 5;
...
var linkBaitCode = '<a href="https://www.gps.ie/car-satnav-gps/">' + linkText + '</ a>';
...
var fullUrl = googleMapsHostUrl + "?" + encodeURI(paramString.replace(/.$/,"")); return fullUrl; } 
redrawMap();